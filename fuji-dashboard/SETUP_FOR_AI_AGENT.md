# fuji-dashboard セットアップ指示書（AIエージェント向け）
...
ユーザーから渡された fuji-dashboard ディレクトリを適切な場所に配置：
...
### Step 4: fuji-dashboard の .env 作成

`<プロジェクトルート>/fuji-dashboard/.env` を作成：
...
A. DB に既存ユーザーがいる。`fuji-dashboard/data.db` を削除すれば再セットアップ可能。
