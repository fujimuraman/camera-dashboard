...
## 13. 返品の扱いと年度確定（利益凍結）（2026-07-02 追加）

中古カメラは1点物（全SKUが qty 0/1、原価は個別法評価）。返品は検品後に**同じSKUで再出品**する運用
（検品・再出品の実務記録は外注センターの管理簿）。

### 13.1 返品計算モデル（設定画面 → 利益計算ロジック）

`settings.profit_return_model`:
- **exclude（既定・推奨）**: 返品された注文は売上・仕入・手数料とも集計から除外。
  返品判定は「注文ID＋SKU」単位で returns テーブルと JOIN するため、
  同一SKUを再出品して再販しても原価の二重計上は起きない
  （1回目の売上は除外され、2回目の売上に原価が1回だけ付く）。
- **subtract_refund**: 総計に含めた上で返金額を利益から控除（プライスター方式）。

### 13.2 年度確定（利益凍結）= 年跨ぎ返品の税務整合

**問題**: exclude モデルは返品日を見ずに元注文を遡って除外するため、
12月販売 → 翌1月返品のケースで「申告済みの前年利益」がダッシュボード上で書き換わってしまう。

**解決**: `settings.profit_closed_until`（YYYY-MM、設定画面 → 利益計算ロジック → 年度確定）。
- この年月**以前**の注文は、後から返品が来ても集計から除外しない（＝申告値を凍結）
- その返品は**返品日の属する期間**に「売上マイナス（返金額）＋売上原価マイナス（原価戻し）」で調整計上
  → 純額（返金額 − 原価）が返品損として当期に乗る。税務の売上値引・返品処理と一致
- 空欄なら無効（従来どおり遡って除外）

**反映範囲**: 累計利益（B/S）・月別P/L・ダッシュボードKPI/日別・売上分析KPI/月別/日別。
曜日別・時間帯別の傾向分析には調整を乗せない（分析ノイズ回避）。

**実装**: app.py の `FROZEN_RETURNS_JOIN_COND`（returns JOIN への凍結条件）と
`_frozen_return_rows()`（調整対象の抽出）。

### 13.3 運用ルール（毎年）

1. 確定申告を提出したら（2月〜3月）、設定画面の「年度確定」に前年12月（例: `2026-12`）を入力
2. 以降、前年分の注文への返品は返品月の返品損として自動計上される
3. TODO管理: 副業シートに毎年登録（2027年分は No.23 登録済み）
